...
class ConfirmButton extends Component {
  ...
  render () {
    ...
    let confirmButton = <button className={this.props.classNameButton} onClick={() => this.onConfirm()} style={buttonWidth}>Aceptar</button>
    if (processing) {
      confirmButton = <button className={'is-disabled ' + this.props.classNameButton} style={buttonWidth}>Procesando...</button>
    }

    const buttons = <div style={{ margin: '0 auto' }}>
      {confirmButton}
      <button className='button' onClick={() => this.hideModal()} style={buttonWidth}>Cancelar</button>
    </div>
